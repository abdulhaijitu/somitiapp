...
import { NoticeStatusBadge } from '@/components/notices';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { Separator } from '@/components/ui/separator';
import { DataTableSkeleton } from '@/components/common/DataTableSkeleton';
import { ArrowLeft, Calendar, Pin, Loader2 } from 'lucide-react';
...
export function NoticeDetailPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-4xl">
      ...
      <Button variant="ghost" onClick={() => navigate(-1)} className="gap-2">
        <ArrowLeft className="h-4 w-4" />
        {language === 'bn' ? 'ফিরে যান' : 'Back to Notices'}
      </Button>

      {/* Notice content */}
      <Card className={`border-border ${notice.is_pinned ? 'border-primary/30 bg-primary/5' : ''}`}>
        <CardHeader>
